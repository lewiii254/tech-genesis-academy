...
const mentors: Mentor[] = [
  ...
  },
  {
    ...
    hourlyRate: 5500,
    avatar:
      "https://images.unsplash.com/photo-1594736797933-d0c64e8fb04e?w=150&h=150&fit=crop&crop=face",
...
const Mentorship = () => {
  ...
  const handleBooking = async () => {
    ...
    const result = await initiatePayment({
      ...
      paymentType: "mentorship_session",
      referenceId: selectedMentor.id,
    });

    if (result) {
      // Start polling for payment status
      pollPaymentStatus(result.paymentId, price);
    } else {
      setPaymentStep("details");
    }
  };

  const pollPaymentStatus = async (paymentId: string, price: number) => {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mentors.map((mentor) => (
            <Card
              key={mentor.id}
              className="bg-white border-slate-200 hover:border-blue-300 transition-all shadow-md"
            >
              <CardHeader>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="border-t border-slate-200 pt-4">
                            ...
                            <Button
                              ...
                              className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white disabled:opacity-50"
                            >
                              {isProcessing || paymentStep === "processing" ? (
                                <>
                            ...
                            <p className="text-xs text-slate-500 text-center mt-2">
                              Secure payment via M-PESA • Session link will be
                              sent via SMS
                            </p>
                          </div>
                        </div>
